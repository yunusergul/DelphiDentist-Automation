...
begin
  ...
  x1Query3.SQL.Clear;
  x1Query3.SQL.Add('select sum(o.ödeme_miktari) as ''fiy'' from hasta_bilgileri h');
...
begin
  verigirisQuery.SQL.Clear; //hasta id için tarama yapýlýyor
  ...
  verigirisQuery.SQL.Clear; // sorun id aranýyor
  ...
  islemquery.SQL.Add('CONVERT(VARCHAR(11), ziy.ziyaret_tarihi, 106)as''Ziyaret Bitiþi'',');
  islemquery.SQL.Add('CONVERT(VARCHAR(5),ziy.ziyaret_tarihi, 108)as''Ziyaret Saati'',yet.yetki+'' ''+kul.kullanici_adi as ''Doktor'',');
  islemquery.SQL.Add('d.dis,hkt.hastalik_adi,ted.tedavi_adi,adbi.yapilan_islem,zid.notlar,ted.fiyat as''Ücret'' from hasta_bilgileri hst');
...
procedure TForm4.buttag(Sender: TObject);   //tag denemesi 1 kullanýlmýyor -
begin
...
procedure TForm4.buttag2(Sender: TObject);   //tag denemesi 2 diþ bilgilerini aktarýcak  kullanýlýyor +
begin
        ADOQuery1.SQL.Clear;
...
begin
        ...
        ADOQuery1.SQL.Clear;
        ADOQuery1.SQL.Add('select hs.adi, hs.soyadi, hs.kimlik_no, ak.adi as ''ak_adi'',ak.soyadi as ''ak_sayadi'',ak.yakýnlýk from hasta_bilgileri hs join akraba ak on hs.akraba_id=ak.id where hs.kimlik_no='''+SearchBox1.Text+'''');
        ADOQuery1.Open;
        ADOQuery2.SQL.Clear;
        if not ADOQuery1.Eof then
        begin
...
begin
        ...
        ADOQuery1.SQL.Clear;
        ADOQuery1.SQL.Add('select hs.id as ''hasta_id'', hs.adi, hs.soyadi, hs.kimlik_no, ak.adi as ''ak_adi'',ak.soyadi as ''ak_sayadi'',ak.yakýnlýk from hasta_bilgileri hs join akraba ak on hs.akraba_id=ak.id where hs.kimlik_no='''+SearchBox1.Text+'''');
        ADOQuery1.Open;
        ADOQuery2.SQL.Clear;
        if not ADOQuery1.Eof then
        begin
...
procedure TForm4.DBGrid3CellClick(Column: TColumn); //teþhis
begin
...
procedure TForm4.DBGrid4CellClick(Column: TColumn); //adýmlar
begin
