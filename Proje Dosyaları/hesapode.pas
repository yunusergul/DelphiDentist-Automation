...
begin
  ADOQuery1.SQL.Clear;
  ADOQuery1.SQL.Add('insert into odeme(ödeme_miktari,hasta_id) values(:ves1,:ves2)');
  ...
  ADOQuery1.SQL.Clear;
  ADOQuery1.SQL.Add('select sum(o.ödeme_miktari) as ''fiy'' from hasta_bilgileri h');
...
begin
 ...
 ADOQuery1.SQL.Add('join sorun_tespiti so on so.ziyaretci_id=zi.id join tedavi te on te.id=so.secilmis_tedavi_id ');
 ADOQuery1.SQL.Add('where h.kimlik_no='+#39+kimlik_no+#39+' group by h.kimlik_no)-(select (sum(o.ödeme_miktari))+0 from hasta_bilgileri h ');
 ...
 ADOQuery1.Open;
  ...
  ADOQuery2.SQL.Clear;
  ADOQuery2.SQL.Add('select d.dis,hkt.hastalik_adi,ted.tedavi_adi,adbi.yapilan_islem,ted.fiyat as''Ücret'' from hasta_bilgileri hst');
